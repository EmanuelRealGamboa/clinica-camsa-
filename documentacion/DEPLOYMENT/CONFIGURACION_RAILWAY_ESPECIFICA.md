...
### No veo los cambios después del push ("sigue igual")

**Causa**: Railway no redesplegó el frontend, el build falló, o el navegador está usando caché.

**Solución**:

1. **Forzar redeploy del frontend**
   - Entra al servicio **bountiful-cat** (frontend) en Railway.
   - Pestaña **Deployments** → deployment más reciente.
   - Comprueba que el commit sea el último (`git log -1 --oneline`).
   - Si no, o si dudas: **"Redeploy"** (o **"Deploy"** → mismo branch).

2. **Revisar que el build terminó bien**
   - En el mismo deployment, abre **"View Logs"**.
   - Debe aparecer `✓ built in ...` y algo como `dist/assets/index-XXXXX.js`. Si hay error de `tsc` o `vite build`, el deploy viejo sigue en vivo.

3. **Comprobar rama y repo**
   - En **Settings → Source**: repositorio `Quint4n4/MenuInteractivo` y rama **main** (o la que uses).
   - Los pushes son `git push railway main`.

4. **Evitar caché en el navegador**
   - **Hard refresh**: `Ctrl + Shift + R` (Windows) o `Cmd + Shift + R` (Mac).
   - O prueba en **ventana de incógnito**.
   - Si sigue igual: DevTools → pestaña **Application** → **Storage** → **Clear site data** para ese dominio.

5. **Confirmar que cargas el bundle nuevo**
   - Abre DevTools → **Console** o **Sources**.
   - El JS debe ser algo como `index-XXXXX.js` (el hash cambia en cada build). Si ves siempre el mismo hash tras un redeploy exitoso, es caché.

6. **Si usas Watch Paths** (`/frontend/**`)
   - En **Settings → Deploy**, el frontend solo se redespliega cuando cambian archivos en `frontend/`.
   - Si aun así no redeploya: deja **Watch Paths** vacío de forma temporal para que cualquier push dispare deploy, o haz **Redeploy** manual.
